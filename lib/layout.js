    ...
    middleware() {
        return async (req, res, next) => {
            ...
            try {
                ...
                // we add a new podiumStream method that sets up streaming and returns the stream
                // object for the developer to work with
                res.podiumStream = (...args) => {
                    const responseStream = new ResponseStream();
                    // pipe the readable response stream into the express writeable res stream
                    // to set up streaming
                    responseStream.pipe(res);
                    // html template changed to accept body as either a string as before
                    // or as in this case, a response stream object.
                    const splitToken = `<!--split-token-->`;
                    const html = this.#view(incoming, splitToken, ...args);
                    const templatePieces = html.split(splitToken);
                    const header = templatePieces[0];
                    const footer = templatePieces[1];

                    console.log(header);
                    console.log(footer);


                    responseStream.send(header);

                    // Once the developer hands back control to Podium, we send the document closing html
                    responseStream.on('done', () => {
                        responseStream.send(footer);
                        responseStream.end();
                    });
