...
import fs from 'fs';
import { PodletStream } from './podlet-stream.js';
        ...
        return async (req, res, next) => {
            ...
            try {
                ...
                res.podiumStream =
                     ...
                     * @param  {...unknown} args
                     */
                    (podlets, layoutMarkup, ...args) => {
                        let podletStream = new PodletStream(incoming);
                        podletStream
                            .from(podlets)
                            .once('beforeStream', (assets) => {
                                incoming.js = assets.js;
                                incoming.css = assets.css;

                                try {
                                    res.status(200);
                                    let document = this.render(
                                        incoming,
                                        layoutMarkup,
                                        ...args,
                                    );
