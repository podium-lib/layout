...
const PodiumLayout = class PodiumLayout {
    ...
    async process(incoming) {
        ...
        const s1 = await this.context.process(incoming);
    ...
    middleware() {
        return async (req, res, next) => {
            ...
            try {
                const result = await this.process(incoming);
                // if no result, then proxy module has taken over and no further work should be done.
                if (!result) return;
